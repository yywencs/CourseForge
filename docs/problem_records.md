...
- 在所有可变校验前按 `round_id + student_id + request_id` 查询原申请，并校验请求指纹；
- 学生身份来自 JWT 上下文，请求体不能指定 `student_id` 或伪造来源；
- 申请状态与本人选课列表查询；
- 先修课程、专业、年级和时间冲突领域策略；
- 退课事务、Redis Lua 名额回补以及 MySQL 可靠修复任务；
- 候补加入、取消、队首抢占与复用选课主链路自动晋级。

仍需明确的基础设施正确性边界是：Redis 数据丢失且消息尚未落库时，基于旧 MySQL
快照重建可能短暂放出正在异步落库的名额。因此生产环境仍要求 Redis AOF、主从高可用
和持久化监控；后续可增加 Redis 整体故障恢复演练。
