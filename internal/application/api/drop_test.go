...
func TestDropEnrollmentPersistsBeforeReleasingProjection(t *testing.T) {
	...
	usecase := NewDropEnrollmentUsecase(repository, projection, repairs)
	...
	if !receipt.MySQLPersisted || !receipt.RedisReleased {
...
func TestDropEnrollmentKeepsRepairPendingWhenRedisFails(t *testing.T) {
	...
	if !receipt.MySQLPersisted || receipt.RedisReleased || repairs.completed != "" {
