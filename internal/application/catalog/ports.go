...
type Repository interface {
	...
	GetRoundForUpdate(context.Context, uint64) (*domain.SelectionRound, error)
	InsertRound(context.Context, *domain.SelectionRound) error
	SaveRound(context.Context, *domain.SelectionRound) error
	RemoveRound(context.Context, uint64) error
	InspectRoundUsage(context.Context, uint64) (domain.SelectionRoundUsage, error)
