...
func TestBindRoundClassUsesDomainPolicyBeforeInsert(t *testing.T) {
	repository := &repositoryStub{
		round: &domain.SelectionRound{ID: 3, TermID: 202601, State: domain.SelectionRoundStatePlanned},
		class: &domain.TeachingClass{ID: 2, TermID: 202602, State: domain.TeachingClassStatePlanned},
	}
	service := NewService(repository)

	err := service.BindRoundClass(context.Background(), 3, 2)
	if !errors.Is(err, domain.ErrTermMismatch) {
		t.Fatalf("BindRoundClass() error = %v, want %v", err, domain.ErrTermMismatch)
	}
...
func TestDeleteRoundUsesUsageFactsBeforeRemove(t *testing.T) {
	repository := &repositoryStub{
		round:      &domain.SelectionRound{ID: 3, TermID: 202601, State: domain.SelectionRoundStatePlanned},
		roundUsage: domain.SelectionRoundUsage{QuotaCount: 1},
	}
	service := NewService(repository)

	err := service.DeleteRound(context.Background(), 3)
	if !errors.Is(err, domain.ErrRoundInUse) {
		t.Fatalf("DeleteRound() error = %v, want %v", err, domain.ErrRoundInUse)
	}
...
type repositoryStub struct {
	...
	roundUsage  domain.SelectionRoundUsage
...
func (r *repositoryStub) ListRounds(context.Context, uint64) ([]SelectionRoundView, error) {
	return nil, nil
}
func (r *repositoryStub) GetRoundForUpdate(context.Context, uint64) (*domain.SelectionRound, error) {
	r.events = append(r.events, "round-lock")
	if r.round == nil {
		return nil, domain.ErrNotFound
	}
	return r.round, nil
}
func (r *repositoryStub) InsertRound(context.Context, *domain.SelectionRound) error { return nil }
func (r *repositoryStub) SaveRound(context.Context, *domain.SelectionRound) error   { return nil }
func (r *repositoryStub) RemoveRound(context.Context, uint64) error {
	r.removeRoundCalled = true
	return nil
}
func (r *repositoryStub) InspectRoundUsage(context.Context, uint64) (domain.SelectionRoundUsage, error) {
	r.events = append(r.events, "round-usage")
	return r.roundUsage, nil
}
func (r *repositoryStub) ListRoundClasses(context.Context, uint64) ([]RoundClassBindingView, error) {
	return nil, nil
}
func (r *repositoryStub) InsertRoundClass(context.Context, uint64, uint64) error {
	r.insertBindingCalled = true
	return nil
}
func (r *repositoryStub) RemoveRoundClass(context.Context, uint64, uint64) error {
	return nil
}
