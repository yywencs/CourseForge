...
// 领域判断由 enrollment 包完成，存储和调度细节由端口实现。
type WaitlistUsecase struct {
	repository WaitlistStore
...
func (u *WaitlistUsecase) Join(
	...
	if u == nil || u.repository == nil || u.admission == nil || u.ids == nil || command == nil {
...
func (u *WaitlistUsecase) PromoteBatch(ctx context.Context, limit int) error {
	if u == nil || u.repository == nil || u.selector == nil || u.observer == nil || limit <= 0 {
	...
	for _, entry := range expired {
		...
		u.observer.WaitlistPromotionCompleted(WaitlistPromotionOutcomeExpired)
	}
	...
	for _, entry := range entries {
		...
		if promoteErr == nil {
			...
			u.observer.WaitlistPromotionCompleted(WaitlistPromotionOutcomePromoted)
			continue
		}
		...
		if decision.Action == enrollment.PromotionFailureActionRetry {
			...
			u.observer.WaitlistPromotionCompleted(WaitlistPromotionOutcomeRetry)
			continue
		}
		...
		u.observer.WaitlistPromotionCompleted(WaitlistPromotionOutcomeCancelled)
	}
	return nil
}
