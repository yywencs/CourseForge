...
import (
	...
	"prizeforge/internal/domain/catalog"
...
func (r roundRow) aggregate() catalog.SelectionRound {
	return catalog.SelectionRound{
		ID: r.ID, TermID: r.TermID, RoundCode: r.RoundCode, RoundName: r.RoundName,
		StartTime: r.StartTime, EndTime: r.EndTime, State: catalog.SelectionRoundState(r.State),
		CreateTime: r.CreateTime, UpdateTime: r.UpdateTime,
	}
}

func (r roundRow) view() applicationcatalog.SelectionRoundView {
	return applicationcatalog.SelectionRoundView{
		ID: r.ID, TermID: r.TermID, RoundCode: r.RoundCode, RoundName: r.RoundName,
		StartTime: r.StartTime, EndTime: r.EndTime, State: catalog.SelectionRoundState(r.State),
...
func (r *Repository) ListRounds(ctx context.Context, termID uint64) ([]applicationcatalog.SelectionRoundView, error) {
	...
	items := make([]applicationcatalog.SelectionRoundView, 0, len(rows))
...
func (r *Repository) GetRoundForUpdate(ctx context.Context, id uint64) (*catalog.SelectionRound, error) {
	var row roundRow
...
func (r *Repository) InsertRound(ctx context.Context, round *catalog.SelectionRound) error {
	row := roundRow{
...
func (r *Repository) SaveRound(ctx context.Context, round *catalog.SelectionRound) error {
	result := r.dbFor(ctx).Model(&roundRow{}).
		Where("id = ? AND state = ?", round.ID, string(catalog.SelectionRoundStatePlanned)).
...
func (r *Repository) RemoveRound(ctx context.Context, id uint64) error {
	result := r.dbFor(ctx).Delete(&roundRow{}, "id = ? AND state = ?", id, string(catalog.SelectionRoundStatePlanned))
	return requireConditionalWrite(result)
}

func (r *Repository) InspectRoundUsage(ctx context.Context, id uint64) (catalog.SelectionRoundUsage, error) {
	db := r.dbFor(ctx)
	var usage catalog.SelectionRoundUsage
	queries := []struct {
		table  string
		target *int64
	}{
	...
	for _, query := range queries {
		if err := db.Table(query.table).Where("round_id = ?", id).Count(query.target).Error; err != nil {
			return catalog.SelectionRoundUsage{}, err
		}
	}
	return usage, nil
}

func (r *Repository) ListRoundClasses(ctx context.Context, roundID uint64) ([]applicationcatalog.RoundClassBindingView, error) {
	var rows []bindingRow
	...
	items := make([]applicationcatalog.RoundClassBindingView, 0, len(rows))
	for _, row := range rows {
		items = append(items, applicationcatalog.RoundClassBindingView{
